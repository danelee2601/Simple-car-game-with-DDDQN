...
import time
import timeit
from car_game import env_replay, env_train
from DDDQN import DQN
...
def train():
    print('Waking up brian cells..')
    sess = tf.Session()

    game = env_train()
    brain = DQN( sess, n_input=4 , n_action=NUM_ACTION ) # n_input = state = [self.x, self.y, self.obs_x, self.obs_y , self.obs_change]
    ...
    #writer = tf.summary.FileWriter('C:\\python64\\envs\\venv\\python64\\logs', sess.graph)
